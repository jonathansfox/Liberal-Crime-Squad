...
#include "../customMaps.h"
 ...
 {  // NOTE: make sure to keep code here matching code in updateworld_laws() in monthly.cpp for when names are changed
	 ...
	 case SITE_LABORATORY_GENETIC:
		 strcpy(loc.name, lastname(true));
		 strcat(loc.name, CONST_locations077);
		 strcpy(loc.shortname, CONST_locations078);
		 break;
	 case SITE_LABORATORY_COSMETICS:
		 strcpy(loc.name, lastname(true));
		 strcat(loc.name, CONST_locations079);
		 strcpy(loc.shortname, CONST_locations080);
		 break;
	 case SITE_BUSINESS_CARDEALERSHIP:
		 generate_name(loc.name, GENDER_WHITEMALEPATRIARCH);
		 strcat(loc.name, CONST_locations081);
		 strcpy(loc.shortname, CONST_locations082);
		 break;
	 case SITE_BUSINESS_DEPTSTORE:
		 strcpy(loc.name, lastname(true));
		 strcat(loc.name, CONST_locations083);
		 strcpy(loc.shortname, CONST_locations084);
		 break;
	 case SITE_INDUSTRY_SWEATSHOP:
		 strcpy(loc.name, lastname(true));
		 strcat(loc.name, CONST_locations086);
	 ...
	 case SITE_BUSINESS_INTERNETCAFE:
		 ...
		 strcpy(loc.shortname, CONST_locations104);
		 break;
	 case SITE_BUSINESS_CIGARBAR:
		 strcpy(loc.name, tag_The);
		 strcat(loc.name, lastname(true));
		 strcat(loc.name, CONST_locations105);
	 ...
	 case SITE_BUSINESS_LATTESTAND:
		 ...
		 strcpy(loc.shortname, CONST_locations108);
		 break;
	 case SITE_OUTDOOR_PUBLICPARK:
		 strcpy(loc.name, lastname(true));
		 strcat(loc.name, CONST_locations109);
		 strcpy(loc.shortname, CONST_locations110);
		 break;
	 }
 }
