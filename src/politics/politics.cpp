...
void elections(char clearformess, char canseethings)
{
	...
	if (canseethings)
	{
		...
		moveAlt(8, 1);
		addstrAlt("The Elections are being held today!");
		getkey();
	}
	//PRESIDENTIAL
	if (year % 4 == 0)
	{
		if (canseethings)
		{
			...
			set_color(COLOR_WHITE, COLOR_BLACK, 1);
			moveZeroZero();
			addstrAlt("Presidential General Election ");
			addstrAlt(year);
		...
		if (canseethings)
		{
			...
			{
				set_color(COLOR_WHITE, COLOR_BLACK, 0);
				moveAlt(8 + stalinmode * 2, 0);
				addstrAlt("Press any key to watch the election unfold.");
				getkey();
			}
			else pause_ms(200);
		}
		...
		if (canseethings)
		{
			set_color(COLOR_WHITE, COLOR_BLACK, 0);
			moveAlt(8 + stalinmode * 2, 0);
			addstrAlt("Press any key to continue the elections.   ");
			getkey();
		}
	...
	if (canseethings)
	{
		...
		set_color(COLOR_WHITE, COLOR_BLACK, 1);
		moveZeroZero();
		addstrAlt("Important Propositions ");
		addstrAlt(year);
	}
	...
	for (p = 0; p < pnum; p++)
	{
		...
		if (canseethings)
		{
			moveAlt(p * 3 + 2, 0);
			set_color(COLOR_WHITE, COLOR_BLACK, 1);
			addstrAlt("Proposition " + tostring(propnums[p]) + ':');
			moveAlt(p * 3 + 2, 18);
			addstrAlt("To ");
	...
	if (canseethings)
	{
		set_color(COLOR_WHITE, COLOR_BLACK, 0);
		moveAlt(23, 0);
		addstrAlt("Press any key to watch the elections unfold.");
		getkey();
	}
	for (p = 0; p < pnum; p++)
	{
		...
		if (canseethings&&recount)
		{
			set_color(COLOR_WHITE, COLOR_BLACK, 1);
			moveAlt(p * 3 + 3, 0);
			addstrAlt("A Recount was Necessary");
		}
...
void supremecourt(char clearformess, char canseethings)
{
	...
	if (canseethings)
	{
		...
		set_color(COLOR_WHITE, COLOR_BLACK, 1);
		moveAlt(8, 1);
		addstrAlt("The Supreme court is handing down decisions!");
		getkey();
	}
	...
	for (c = 0; c < cnum; c++)
	{
		...
		if (canseethings)
		{
			...
			{
				lastname(name2, true);
				strcat(name2, pickrandom(corporateSuffix));
			}
			addstrAlt(name1);
			addstrAlt(" vs. ");
			addstrAlt(name2);
			moveAlt(c * 3 + 3, 0);
			addstrAlt("A Decision could ");
	...
	if (canseethings)
	{
		set_color(COLOR_WHITE, COLOR_BLACK, 0);
		moveAlt(23, 0);
		addstrAlt("Press any key to watch the decisions unfold.");
		getkey();
	}
	for (c = 0; c < cnum; c++)
	{
	...
	if (LCSrandom(10) >= 6)
	{
		...
		if (canseethings)
		{
			...
			set_color(COLOR_WHITE, COLOR_BLACK, 0);
			moveAlt(7, 0);
			addstrAlt("Press any key to see what happens.");
			getkey();
		}
		...
		{
			moveAlt(4, 0);
			addstrAlt("After much debate and televised testimony, a new justice,");
			moveAlt(5, 0);
...
void congress(char clearformess, char canseethings)
{
	...
	if (canseethings)
	{
		...
		set_color(COLOR_WHITE, COLOR_BLACK, 1);
		moveAlt(8, 1);
		addstrAlt("Congress is acting on legislation!");
		getkey();
	}
	//CHANGE THINGS AND REPORT
	if (canseethings)
	...
	if (canseethings)
	{
		set_color(COLOR_WHITE, COLOR_BLACK, 0);
		moveAlt(23, 0);
		addstrAlt("Press any key to watch the votes unfold.");
		getkey();
		moveAlt(0, 62);
		addstrAlt("House");
		moveAlt(0, 70);
	...
	if (havebill)
	{
		if (canseethings)
		{
			set_color(COLOR_WHITE, COLOR_BLACK, 0);
			moveAlt(23, 0);
			addstrAlt("Press any key to watch the President.                   ");
			getkey();
			moveAlt(0, 35);
			addstrAlt("President");
			pause_ms(500);
	...
	else if (canseethings)
	{
		set_color(COLOR_WHITE, COLOR_BLACK, 0);
		moveAlt(23, 0);
		addstrAlt("None of the items made it to the President's desk.");
		moveAlt(24, 0);
		addstrAlt(pressKeyToReflect + "    ");
		getkey();
	}
