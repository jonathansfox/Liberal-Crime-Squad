			...
			else if (getcliptype(cliptype) != -1) //Must be a clip type too.
			{
				...
				if (i == len(attacks))
				{
					xmllog.log("In " + owner + ", " + cliptype +
						"can not be used by " + weapontype + singleDot);
					cliptype = NONE;
				}
			}
			// Undefined clip type.
	...
	infiltration_(0), juice_(0), money_(20, 40)
{
	...
	if (!len(type_name_))
	{
		xmllog.log("type_name not defined for " + idname_ + singleDot);
...
std::string CreatureType::get_type_name() const
{
	...
	case CREATURE_WORKER_SERVANT:
		if (lawList[LAW_LABOR] == -2 && lawList[LAW_CORPORATE] == -2) return "Slave";
		break;
	case CREATURE_WORKER_JANITOR:
		if (lawList[LAW_LABOR] == 2) return "Custodian";
		break;
	case CREATURE_WORKER_SWEATSHOP:
		if (lawList[LAW_LABOR] == 2 && lawList[LAW_IMMIGRATION] == 2) return "Migrant Worker";
		break;
	case CREATURE_CARSALESMAN:
		if (lawList[LAW_WOMEN] == -2) return "Car Salesman";
		break;
