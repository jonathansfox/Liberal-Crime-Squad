...
void makecreature(Creature &cr, short type)
{
	...
	case CREATURE_PSYCHOLOGIST:
		...
		if (cr.gender_liberal == GENDER_MALE || LCSrandom(2))
			cr.give_armor(*armortype[getarmortype(tag_ARMOR_CHEAPSUIT)], NULL);
		else
			cr.give_armor(*armortype[getarmortype(tag_ARMOR_CHEAPDRESS)], NULL);
		break;
	case CREATURE_NURSE:
		if (lawList[LAW_GUNCONTROL] == -2 && !LCSrandom(3))
		{
	...
	case CREATURE_CCS_VIGILANTE:
		cr.give_armor(*armortype[getarmortype(tag_ARMOR_CLOTHES)], NULL);
		...
		case 5:
		{
			...
			cr.take_clips(*cliptype[getcliptype(tag_CLIP_ASSAULT)], 7);
			cr.give_armor(*armortype[getarmortype(tag_ARMOR_CIVILLIANARMOR)], NULL);
			break;
		}
		case 6:
		{
			cr.give_weapon(*weapontype[getweapontype(tag_WEAPON_SEMIRIFLE_AR15)], NULL);
			cr.take_clips(*cliptype[getcliptype(tag_CLIP_ASSAULT)], 7);
			cr.give_armor(*armortype[getarmortype(tag_ARMOR_ARMYARMOR)], NULL);
			break;
		}
		default:
		{
	...
	case CREATURE_SOCIALITE:
		cr.give_armor(*armortype[getarmortype(cr.gender_liberal == GENDER_FEMALE ? tag_ARMOR_EXPENSIVEDRESS : tag_ARMOR_EXPENSIVESUIT)], NULL);
		break;
	case CREATURE_THIEF:
		switch (LCSrandom(5))
		{
