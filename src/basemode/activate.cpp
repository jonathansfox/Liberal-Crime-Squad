...
void activate()
{
	...
	while (true)
	{
		...
		moveOneZero();
		addstrAlt("ÄÄÄÄCODE NAMEÄÄÄÄÄÄÄÄÄÄÄÄSKILLÄÄÄHEALTHÄÄÄLOCATIONÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ");
...
void activatebulk()
{
	...
	while (true)
	{
		...
		addstrAlt("Activate Uninvolved Liberals");
		moveOneZero();
		addstrAlt("ÄÄÄÄCODE NAMEÄÄÄÄÄÄÄÄÄÄÄÄCURRENT ACTIVITYÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ");
...
void recruitSelect(Creature &cr)
{
	...
	while (true)
	{
		...
		set_color(COLOR_WHITE, COLOR_BLACK, 0);
		moveOneZero();
		addstrAlt("ÄÄÄÄTYPEÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄDIFFICULTY TO ARRANGE MEETINGÄÄ");
		int y = 2, difficulty;
...
void select_augmentation(Creature *cr) //TODO: Finish and general cleanup
{
	...
	while (true)
	{
		...
		case 0: //PAGE 0, selecting a liberal
			...
			set_color(COLOR_WHITE, COLOR_BLACK, 0);
			moveOneZero();
			addstrAlt("ÄÄÄÄNAMEÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄHEALTHÄÄÄÄÄÄÄÄÄÄÄÄHEARTÄÄÄÄÄÄÄÄAGEÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ");
...
void select_makeclothing(Creature *cr)
{
	...
	while (true)
	{
		...
		set_color(COLOR_WHITE, COLOR_BLACK, 0);
		moveOneZero();
		addstrAlt("ÄÄÄÄNAMEÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄDIFFICULTYÄÄÄÄÄÄÄÄÄÄÄÄÄCOSTÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ");
		int y = 2, difficulty;
...
char select_view(Creature *cr, int &v)
{
	...
	while (true)
	{
		...
		set_color(COLOR_WHITE, COLOR_BLACK, 0);
		mvaddstrAlt(2, 0, "ÄÄÄÄTOPICÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄINTERESTÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ");
		int y = 3, x = 0;
